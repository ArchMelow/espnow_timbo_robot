    ...
    for m in net_table[1:]:
        ...
        returned_str += '''<tr>
                ...
                <td>
                    <form action="#" method="POST">
                        <input type="hidden" name="device" value="Timbo_{hex_m}">
                        <input type="hidden" name="button" value="REVERSE-PLAY">
                        <button type="submit" class="button">REVERSE-PLAY</button>
                    </form>
                </td>
...
def web_page(access_ip_str = "", net_table = None):
    ...
    html_page = '''<!DOCTYPE html>
        ...
        <form action="#" method="POST">
                <input type="hidden" name="button" value="SHARE-WIFI">
                <button type="submit" class="button">SHARE-WIFI</button>
        </form>
        
        
        <form action="#" method="POST">
                <input type="hidden" name="button" value="EXIT">
...
def run_APserver(runner_obj):
    ...
    while True:
        ...
        # share WiFi to the ESPNow group.
        if request.find('SHARE-WIFI') != -1:
            runner_obj.mb.e.send(None, json.dumps({'tag':'share_wifi',
                                                   'wifi':runner_obj.mb.shared_wifi_conf,
            ...
            if device_mac and action == 'REVERSE-PLAY':
                
                runner_obj.mb.e.send(device_mac, json.dumps({'tag':'reverse_play',
                                                             'is_queen':runner_obj.mb.is_queen}))
