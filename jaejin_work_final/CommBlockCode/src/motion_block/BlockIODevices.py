from machine import Pin, ADC, PWM, Timer, enable_irq, reset, I2C, UART
...
class BlockIODevices:
    ...
    def __init__(self, is_queen = False):
        ...
        self.led_green = PWM(Pin(21, Pin.OUT))
        ...
        self.cpu_on_pin = Pin(22, Pin.OUT)
        
        self.cpu_on_pin.value(1) 
        
        self.bttn = Pin(18, Pin.IN, Pin.PULL_UP)
        
        
        self.backward_motor = Pin(33, Pin.OUT)
        
        self.forward_motor = Pin(32, Pin.OUT)
        self.nSleep = Pin(25, Pin.OUT)
        #print('sdfsdfs')
        self.adc = ADC(Pin(35), atten=ADC.ATTN_6DB) # analog-to-digital conversion on Pin 36 (MODIFIED)
        ...
        # UART
        self.uart = UART(2, baudrate=9600, bits=8, parity=None, stop=1, rx=16, tx=17)
        ...
        self.shared_wifi_conf = None
        
        try: 
            with open('./conf/wifi.conf', 'r') as f:
                read_data = f.read()
                print(read_data)
                self.shared_wifi_conf = json.loads(read_data)
        except:
            print('wifi.conf does not exist.')
    ...
    def block_power_if_idle(self):
        if self.kill_timer and self.kill_timer % 500000 == 0:  #10000:20 sec, 200000
