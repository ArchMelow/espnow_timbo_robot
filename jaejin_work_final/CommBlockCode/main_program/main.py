...
from src.firmate.firmate import firmate
...
class Runner:
        ...
        while True:
            ...
            if json_msg['is_queen']:
                if json_msg['tag'] == 'distribute':
                    ...
                    self.mode = 1
                    self.is_grouped = True # comm block is grouped.
    ...
    def button(self):   #LEE
        bttn_val = 0 if self.mb.bttn.value() else 1
    ...
    async def runner(self):
        ...
        while True:
            ...
            if self.mode == 0: # waiting mode
                ...
                # if grouped already, move on to mode 1.
                if self.is_grouped:
                    self.mode = 1
                    continue
            ...
            if self.mode == 1:
                ...
                # checking for button is not available.
                
                # wait for the serial response (5 sec)
                while self.serial_cnt <= 700:
                    self.mb.uart.write('Start connection')
                    if self.mb.uart.any():
                        self.serial_msg = self.mb.uart.read()
                        break
                    #print(self.serial_cnt)
                    self.serial_cnt += 1
                
                if self.serial_msg is not None:
                    print('Enter Firmate mode')
                    firmate(self)
            ...
            if self.mode == 2:
                ...
                # not a good practice, but doesn't matter
                self.serial_msg = None
                self.serial_cnt = 0
