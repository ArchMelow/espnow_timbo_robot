...
def generate_table(net_table):
    table_rows = ""
    for ssid, _, _, _, _, _ in net_table:
        table_rows += "<tr>"
        table_rows += f"<td>{ssid.decode('utf-8')}</td>"
        table_rows += "<td>"
        table_rows += f"<form action='/connect' method='POST'>"
        table_rows += f"<input type='hidden' name='ssid' value='{ssid.decode('utf-8')}' />"
        table_rows += "Password: <input type='password' name='password' required>"
        table_rows += "<input type='submit' value='Connect'>"
        table_rows += "</form>"
        table_rows += "</td>"
        table_rows += "</tr>"
    return table_rows
...
def web_page(access_ip_str="", scanned_list=None):
    generated_str = generate_table(scanned_list)
    ...
    html_page = '''<!DOCTYPE html>
        ...
        '''.format(access_ip_str=access_ip_str, generated_str=generated_str)
    return html_page
...
def configure_wifi():
    ...
    while True:
        ...
        if ssid_match and password_match:
            ssid_value = ssid_match.group(1).split('&')[0]
            password_value = password_match.group(1)
            
            if not ssid_value or not password_value:
                print('ssid or password not valid. Try again.')
                continue
        ...
        response = web_page(access_ip_str, scanned)
