...
class BlockIODevices:
    ...
    def __init__(self, is_queen = False):
        ...
        # if user toggles the button twice quickly, the record mode will be skipped.
        # i.e. the memory buffer is not cleared.
        self.stay_on_record_mode_cnt = 0
        self.slave_stay_on_record_mode_cnt = 0
    ...
    def write_shared_wifi_config(self):
        if self.shared_wifi_conf is not None:
            with open('./conf/wifi.conf', 'w') as f:
                f.write(json.dumps(self.shared_wifi_conf))
            self.shared_wifi_conf = None # reset
