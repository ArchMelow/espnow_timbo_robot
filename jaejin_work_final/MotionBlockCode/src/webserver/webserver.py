...
def open_server(runner_obj):
    ...
    while True:
        ...
        if (method == 'POST' and path == '/upload') or \
            (method == 'POST' and path == '/update') or \
            (method == 'POST' and path == '/model'):
            ...
            if path == '/model':
                # you have to upload a file named model.tflite.
                filename = s2.decode().split(';')[-1].split('=')[-1]
                filename = './model/' + filename[1:-3].strip()
