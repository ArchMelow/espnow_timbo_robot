...
from src.ai.ai_app import AI_app
...
class Runner:
    ...
    def recv_cb_queen(self, e):
        while True:
            ...
            # every time we receive message, reset the reverse play flag
            #self.reverse_play = False
            ...
            # save the wifi info that the comm block has sent to the blocks
            if json_msg['tag'] == 'share_wifi':
                self.mb.shared_wifi_conf = json_msg['wifi']
                self.mb.received_msg = None
                self.mb.received_mac = None    
            
            # check the mode change of playing the memory in reverse order.
            # same for both the queen and the slaves.
            if json_msg['tag'] == 'reverse_play':
        ...
        while True:
            ...
            if json_msg['is_queen']:
                ...
                if json_msg['tag'] == 'record':
                    ...
                    self.mb.received_msg = None
                    self.mb.received_mac = None
                # append to memory only when the slave stays on record mode (considers double toggling)
    ...
    def button(self):   #LEE
        ...
        if bttn_val and not self.prev_bttn:
            ...
            self.longpress_cnt = 0
            self.mode = (self.mode + 1) % 5 if self.mode < 5 else 0 # mode 3, 4 is unreachable
    ...
    def detect_mode_change(self):    
        ...
        if self.prev_mode != self.mode:
            ...
            self.mb.stay_on_record_mode_cnt = 0
            self.mb.slave_stay_on_record_mode_cnt = 0
            ...
            if self.mode == 2: # turn on the motor
                ...
                if len(self.mb.net_table) > 1 and self.mb.is_queen:
        ...
        # if the button was not pressed for cnt < 150 in record mode, increment until cnt == 150
        # this only executes once when the mode is changed.
        elif self.mode == 1:
            if self.mb.stay_on_record_mode_cnt < 255:
                self.mb.stay_on_record_mode_cnt += 1
            if self.mb.stay_on_record_mode_cnt == 250:
    ...
    async def runner(self):
        ...
        while True:
            ...
            self.mb.write_shared_wifi_config()    
            ...
            if self.mode == 0: # idle mode
                ...
                if self.longpress_cnt == 700:
                    ...
                    self.mb.set_led_color('o') # turn on orange LED (grouping..)
                    grouping_res = grouping(self)
                    self.mb.set_led_color('none') # grouping finished
            ...
            if self.mode == 1: # record mode
                ...
                # consider when this mode is skipped (toggled twice quickly)
                if self.mb.stay_on_record_mode_cnt >= 250:
            ...
            if self.mode == 2: # play mode
                ...
                # currently used memory index (slot)
                i = self.mb.cur_mem_idx
                
                # play in reverse?
                mem = self.mb.duty_memories[i] if self.reverse_play else self.mb.duty_memories[i][::-1]
                
                
                if not mem or (self.duty_mem_idx >= len(mem)): # no memory (empty)
                    self.button()
                else:
                    self.button()
                    await self.mb.async_move_motor(int(mem[self.duty_mem_idx]))
                    if self.longpress_cnt == 700:
                        print('bttn held for long time.. do nothing')
                    if self.p_flag == 1:      #LEE
                        self.duty_mem_idx = (self.duty_mem_idx + 1) % len(mem)
                        self.p_flag = 0
            
            if self.mode == 3: # AI mode
                self.button()
