...
class Runner:
    ...
    def button(self):   #LEE
        ...
        if bttn_val and not self.prev_bttn:
            ...
            self.longpress_cnt = 0
            self.mode = (self.mode + 1) % 4 if self.mode < 4 else 0 # mode 3, 4 is unreachable
    ...
    async def runner(self):
        ...
        while True:
            ...
            if self.mode == 1: # record mode
                ...
                if self.mb.stay_on_record_mode_cnt >= 150:
            ...
            if self.mode == 3: # RL mode
                ...
                # send message to the comm. block
                ax, ay, az = self.mb.lsm.read_accel()
                mx, my, mz = self.mb.lsm.read_magnet()
                gx, gy, gz = self.mb.lsm.read_gyro()
                
                
                #print(self.mb.net_table)
                if len(self.mb.net_table) > 1:                    
                    self.mb.e.send(None, json.dumps({'is_queen':self.mb.is_queen,
                                                     'ax' : ax, 'ay' : ay, 'az' : az,
                                                     'mx' : mx, 'my' : my, 'mz' : mz,
                                                     'gx' : gx, 'gy' : gy, 'gz' : gz,
                                                     'tag' : 'to_comm'}))
